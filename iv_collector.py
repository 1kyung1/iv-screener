...
def get_last_trading_date(ref_date: date) -> date:
    """ref_date가 거래일이면 그대로, 아니면 가장 최근 거래일 반환"""
    check = ref_date
    for _ in range(7):
        if is_market_open(check):
            return check
        check -= timedelta(days=1)
    return ref_date

today_date = get_last_trading_date(et_date)
    ...
    try:
        ...
        for idx_sym, idx_col in [("SPY", "spy"), ("QQQ", "qqq"), ("IWM", "iwm")]:
            try:
                ...
                # Step 1: yfinance에서 (만기, strike) → OI 맵 구성
                # Alpaca에 여러 만기가 섞여 있으므로 만기별로 분리해서 구성
                # key: (exp_str "YYMMDD", strike) → oi
                yf_call_oi = {}  # {("260717", 741.0): 1590, ...}
                yf_put_oi  = {}
                try:
                    idx_ticker  = yf.Ticker(idx_sym)
                    valid_exps  = []
                    ...
                    for exp in valid_exps:
                        exp_key = pd.Timestamp(exp).strftime("%y%m%d")  # "260717"
                        yf_chain = idx_ticker.option_chain(exp)
                        for _, r in yf_chain.calls.iterrows():
                            k = (exp_key, round(float(r["strike"]), 1))
                            yf_call_oi[k] = float(r["openInterest"] or 0)
                ...
                try:
                    ...
                    for opt in filtered:
                        ...
                        opt_type = opt.symbol[-9]
                        try:
                            # Alpaca 심볼: ex) SPY260717C00741000
                            # 만기키: 심볼[-15:-9] = "260717"
                            # strike: 심볼[-8:] / 1000
                            exp_key = opt.symbol[-15:-9]
                            strike  = round(int(opt.symbol[-8:]) / 1000, 1)
