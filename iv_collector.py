...
def collect_market_data():
    try:
        ...
        # Alpaca 그릭스 우선, 없으면 yfinance gamma 컬럼으로 폴백
            ...
            try:
                ...
                try:
                    ...
                    if call_gex_total != 0.0 or put_gex_total != 0.0:
                        calculated = True
                        print(f"    [{idx_sym} GEX Alpaca] call={round(call_gex_total,2)} put={round(put_gex_total,2)}")
                except Exception as e:
                ...
                if not calculated:
                    ...
                    if target_exp:
                        ...
                        if "gamma" in yf_chain.calls.columns:
                            ...
                            calculated = True
                            print(f"    [{idx_sym} GEX yfinance] call={round(call_gex_total,2)} put={round(put_gex_total,2)}")
...
def collect_market_data():
    try:
        ...
        # ✅ 주요 지수 GEX: SPY, QQQ, IWM (Alpaca 옵션 체인 기반)
        for idx_sym, idx_col in [("SPY", "spy"), ("QQQ", "qqq"), ("IWM", "iwm")]:
            try:
                idx_price = row.get(f"{idx_col}_close")
                if not idx_price:
                ...
                row[f"{idx_col}_gex_put"]  = round(put_gex_total,  2)
                row[f"{idx_col}_gex"]      = round(call_gex_total - put_gex_total, 2)
                print(f"    [{idx_sym} GEX] call={round(call_gex_total,2)} put={round(put_gex_total,2)} net={round(call_gex_total-put_gex_total,2)}")
            except Exception as e:
                print(f"    [{idx_sym} GEX 실패] {e}")
